...
Use the `v0.1.0-beta5` tag for the release source, or record the commit of `main` when building newer changes. Local builds have their own checksums and do not inherit the project signature. Whole-image byte-for-byte reproducibility is not established.
...
cd "$JV_WORK/repo"
export JV_REPO="$PWD"
export JV_TAG=0.1.0-beta5-local1
...
| Output | Purpose |
|---|---|
| `dist/justverify-0.1.0-beta5-local1.img` | Extracted image to select in balenaEtcher |
| `dist/justverify-0.1.0-beta5-local1.img.xz` | Compressed image for download/storage |
| `dist/justverify-0.1.0-beta5-local1-SHA256SUMS` | Both file hashes; check from `dist/` |
| `dist/justverify-0.1.0-beta5-local1.layout.json` / `.size-audit.json` | Partition layout and size audit |
