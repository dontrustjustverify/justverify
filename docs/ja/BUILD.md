...
i2pdは`catalog/i2pd.json`で固定したソースを検証してビルドします。バンドルには原本ソース、BSDライセンス、公開reseed証明書を含みます。イメージ作成の第7引数は必須で、固定版のBoost program-optionsランタイムもインストールします。ルーター秘密鍵は含みません。
